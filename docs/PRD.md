...
version: 0.6
status: fixture candidate comparison and school-form draft export validated
owner: 이수미 교수
created: 2026-07-13
updated: 2026-07-14
---
...
| 항목            | 내용                                                        |
| --------------- | ----------------------------------------------------------- |
| 제품명          | 수업로 AI(Sup-Ro AI)                                        |
| 첫 서비스       | AI 체험학습 코디네이터                                      |
| 저장소 식별자   | `sumilee-pcu/Sup-Ro`                                        |
| 문서 버전       | 0.6                                                         |
| 상태            | fixture 후보 비교·선택과 학교 양식 작성용 초안 검증 완료    |
| 기준일          | 2026-07-14                                                  |
| 제품 책임자     | 이수미 교수                                                 |
...
- 장소 고정, 제외, 대체 요청
- 필수조건을 먼저 판정하는 후보별 적합도·근거·주의사항 순위
- 후보 1~3곳 비교·복수 선택과 선택 장소만 일정에 반영
...
- 원당중학교 2025 개인 교외체험학습 양식의 비개인정보 항목을 매핑한 작성용 초안
...
- `Sup-Ro` Kakao 개발자 앱과 localhost 도메인을 2026-07-14 등록하고 지도 제품을 활성화했으며, Maps SDK와 Local 검색의 실제 연결을 각 1회 검증했다.
- 현재 계정에서 `Sup-Ro`가 첫 지도 활성 앱이고 유료 API·BizWallet은 꺼져 있다. 2026-07-21 신규 경로 API의 최종 명세를 확인하기 전에는 대중교통·도보·자전거 어댑터를 완료 처리하지 않는다.
...
| M4 평가·배포     | 12개 벤치마크, 평가 대시보드, 배포 문서  | 12/12 평가와 공개 fixture 서비스    | 완료 |
...
- [x] 작업 브랜드를 `수업로 AI(Sup-Ro AI)`로 단축 확정하고 네이밍 조사 기록
- [x] NCIC 2022 개정 교육과정을 초기 기준 출처로 선정하고 시연용 요약을 원문과 분리
- [x] 가상 장소와 합성 경로를 사용한 중2 과학 fixture 시나리오 구축
- [ ] 공개 전 상표·도메인·스토어 이름 최종 확인
- [x] Kakao Maps 개발자 앱·localhost/공개 HTTPS 도메인·현재 쿼터·요금·비과금 설정 확인
- [ ] 2026-07-21 Kakao 경로 API 최종 명세와 무료 적용 상태 재확인
- [ ] TourAPI·기상·환경·안전 데이터 키 발급 가능 여부 확인
- [ ] 대표 교육과정 원문과 성취기준 색인의 사용·출처표시 방식 확정
- [ ] 첫 공개 데모 지역과 대표 장소 5~8개 선정
- [x] 원당중학교 2025 학교장허가 교외체험학습 HWPX 원본과 필드 구조 확인
- [ ] 개인 교외체험학습 신청서가 단체 현장수업 운영계획서를 대체할 수 있는지 학교 확인
